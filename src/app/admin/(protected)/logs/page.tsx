...
import { Search, User, Store, Star, Shield, Trash2, Clock, Loader2 } from 'lucide-react'
    ...
    const fetchLogs = async () => {
        try {
            const [users, providers, reviews] = await Promise.all([
                (supabase as any).from('users').select('id, name, email, created_at').order('created_at', { ascending: false }).limit(20),
                (supabase as any).from('providers').select('id, business_name, created_at').order('created_at', { ascending: false }).limit(20),
                (supabase as any).from('reviews').select('id, rating, created_at').order('created_at', { ascending: false }).limit(20)
            ])
